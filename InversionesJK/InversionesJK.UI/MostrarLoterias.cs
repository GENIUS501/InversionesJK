...
{
    public partial class MostrarLoterias : Form
    {
        #region Variables
        ...
        int valorcelda = -1;
        #endregion
        public MostrarLoterias()
        {
            InitializeComponent();
        }

        private void MostrarLoterias_Load(object sender, EventArgs e)
        {
            try
            {
                NLoterias Negocios = new NLoterias();
                ELoterias Entidad = new ELoterias();
        ...
        private void Renderizar(string Accion)
        {
            try
            {
                ...
                if (Accion != "A" && Accion == "M" && valorcelda != -1 || Accion != "A" && Accion == "E" && valorcelda != -1 || Accion != "A" && Accion == "C" && valorcelda != -1)
                {
                    ...
                    AgregarLoterias frm = new AgregarLoterias();
                    frm.Accion = Accion;
                    frm.Id = valorcelda;
                    frm.Usuario = Usuario;
                    frm.ShowDialog();
                    valorcelda = -1;
                    MostrarLoterias_Load(null, null);
                }
                else
                {
                    if (Accion == "A")
        ...
        private void btn_eliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (valorcelda != -1)
                {
                    DialogResult dr = MessageBox.Show("Realmente desea eliminar la loteria?", "Eliminar la loteria", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (dr == DialogResult.Yes)
                    {
                        ...
                        if (FilasAfectadas > 0)
                        {
                            MessageBox.Show("Loteria eliminada satisfactoriamente", "Excelente", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("Error al eliminar la loteria!!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                        valorcelda = -1;
                        MostrarLoterias_Load(null, null);
                    }
                ...
                {
                    MessageBox.Show("Debe seleccionar una loteria!!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception)
            {
        ...
        private void btn_buscar_Click(object sender, EventArgs e)
        {
            try
            {
                errorProvider1.SetError(txt_buscar_loteria, "");
                if (this.txt_buscar_loteria.Text != "")
                {
                    var IdLoteria = int.Parse(this.txt_buscar_loteria.Text);
                    NLoterias Negocios = new NLoterias();
                    dat_principal.DataSource = Negocios.Mostrar().Where(x => x.ID_loteria == IdLoteria).ToList();
                }
                else
                {
                    errorProvider1.SetError(txt_buscar_loteria, "Debe llenar este campo!!");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btn_buscar_nombre_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    NLoterias Negocios = new NLoterias();
                    dat_principal.DataSource = Negocios.Mostrar().Where(x => x.Nombre_loteria.Contains(this.txt_nombre.Text.ToString())).ToList();
                }
                else
                {
