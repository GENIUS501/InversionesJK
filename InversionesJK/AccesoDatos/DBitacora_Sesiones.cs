    ...
    public class DBitacora_Sesiones
    {
        ...
        public int Agregar(EBitacora_Sesiones obj)//Viene de la vista Obj
        {
            try
            {
                ...
                {
                    Bitacora_ingreso_salida Objbd = new Bitacora_ingreso_salida();//Viene de la base de datos
                    Objbd.Id_Usuario = obj.Id_Usuario;
                    Objbd.fecha_hora_ingreso = obj.fecha_hora_ingreso;
                    db.Bitacora_ingreso_salida.Add(Objbd);
        ...
        public int Modificar(EBitacora_Sesiones Obj)
        {
            try
            {
                using (TransactionScope Ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    var Objbd = db.Bitacora_ingreso_salida.Where(x => x.codigo_ingreso_salida == Obj.codigo_ingreso_salida).FirstOrDefault();
        ...
        public List<EBitacora_Sesiones> Mostrar()
        {
            try
            {
                ...
                Obj = db.Bitacora_ingreso_salida.Select(x => new EBitacora_Sesiones
                {
